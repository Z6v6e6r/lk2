...
function withHostAvatarUrls(
  ...
  if (!source?.readAvatarSource) return activities;
  return activities.map((activity) =>
    activity.host && source.readAvatarSource?.(activity.id)
      ? {
          ...activity,
          host: {
            ...activity.host,
...
export function registerBookingRecommendationRoutes(
  ...
  app.post(
    ...
    async (request, reply) => {
      ...
      if (
        ...
        (body.screen !== 'FOR_ME' && body.screen !== 'MY_BOOKINGS')
      ) {
        return sendApiError(
          request,
          reply,
      ...
      return {
        ...
        concurrency: job.screen === 'FOR_ME' ? 3 : 1,
      };
    },
  );
    ...
    async (request, reply) => {
      ...
      try {
        if (command.operation === 'schedule.read') {
          result = {
            kind: 'schedule',
            activities: normalizeVivaExerciseRecommendationPayload(body.payload).filter(
              (activity) => moscowDate(new Date(activity.startsAt)) === command.date,
            ),
          };
        } else {
  ...
  app.get(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      const sourceUrl = options.exerciseSource?.readAvatarSource?.(activityId);
      ...
      try {
        ...
        request.log.error({ error, activityId }, 'booking activity host avatar delivery failed');
        return sendApiError(
          request,
          reply,
          503,
