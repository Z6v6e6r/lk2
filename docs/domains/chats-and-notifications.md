...
проверяют current permission, active membership и block policy. Участник, у которого нет хранимого
`chat.direct.create`, не может открыть ни один direct-чат, поэтому создание нового диалога с ним
отказывает стабильным `CHAT_PARTICIPANT_CHAT_ACCESS_REQUIRED` и не пишет ни тред, ни
`messaging.conversation.created.v1`: событие не публикуется для того, кто на момент создания не смог бы
открыть тред. Отзыв гранта между публикацией события и проекцией уведомления не перепроверяется —
проектор проверяет только статус учётной записи получателя, поэтому такая ссылка может устареть.
Различимый отказ выбран намеренно, как и существующий `CHAT_PARTICIPANT_UNREACHABLE`: он раскрывает
только состояние прав авторизованного клиента с доступом к чатам, а не персональные данные.
Хранимый грант сводится к клиентскому каталогу беты в момент выдачи client-токена, пока действует
`BETA_FULL_CLIENT_ACCESS_ENABLED`: `identity.user_access_profiles` только пополняется, роли и
admin-only права не меняются, а аккаунт без строки получает её. Так аккаунты, зарегистрированные
после последнего операторского `user:access:beta-full`, перестают держать пустой профиль при
непустых claims токена. Сходимость наступает при следующей выдаче client-токена (вход или refresh
сессии), поэтому до неё `CHAT_PARTICIPANT_CHAT_ACCESS_REQUIRED` остаётся корректным описанием
состояния гранта партнёра, а операторский backfill по-прежнему ускоряет разблокировку. Пока флаг
включён, сужение любого из бета-прав через `set-user-access` снимается на следующей выдаче
client-токена: хранимый профиль — то, что читают гейты чатов и сообществ. Неудачная запись
сходимости не отменяет выдачу токена — он подписывается из хранимого профиля, как до сходимости, —
иначе сбой записи после ротации refresh-токена стоил бы аккаунту повторного входа.
Уже существующая каноническая пара гейтом не затрагивается, поэтому потеря гранта одной стороной не
отбирает принятый membership. `chatPolicy` участника — правило
...
`integration.external_entity_map` (`LK_LEGACY_SNAPSHOT`/`game_station`). Вкладка показывает **все опубликованные станции** как
направления переписки, а не только те, куда зритель уже писал: первое сообщение создаёт его
собственный диалог со станцией (один диалог на зрителя и станцию), станция без истории открывает
пустой тред, а диалог, который не маппится на опубликованную станцию, остаётся видимым, чтобы не
терять историю. Срез выключен по умолчанию
(`SUPPORT_STATIONS_ENABLED=false`), живёт вне контракта разговоров (без realtime, политики
уведомлений и курсора прочтения) и не пишет локальное бизнес-состояние: идемпотентность
отправки держит сам провайдер по `externalMessageId`. Фотографии в этом контуре идут через тот же
media bucket: входящая картинка оператора материализуется в WebP при чтении истории (inline base64
разбирается локально, ссылка скачивается только с хоста `SUPPORT_LEGACY_BASE_URL`), исходящая
загружается через `POST /support/attachments` и уходит в CUP inline data URL вместе с
`attachmentIds`; ключ объекта всегда собирается из идентичности читателя, а браузер получает только
короткоживущий redirect. Сообщение отдаёт `authorName` — имя оператора, которое сохранил провайдер,
поэтому в ЛК видно, кто отвечает, а не безликая «Станция». Перенос станционных обращений в нативные
...
5. Повтор с тем же ключом/`clientMessageId` в том же диалоге возвращает исходный результат, а не
   создаёт сообщение; повторное использование `clientMessageId` этим tenant user в другом диалоге
   отклоняется как idempotency conflict.
...
2. `POST /admin/api/v1/{tenantKey}/notifications/recipients/resolve` принимает два селектора —
   ...
   разной силой доказательства: всегда побеждает подтверждённый телефон
   (`profile.user_summaries.phone_e164`); его пишет либо вход по телефону, либо отдельное
   подтверждение телефона для уже авторизованного аккаунта, а provider-привязка
...
маршруту `/notifications`, а не к усечённому адресу. Категория шаблона задаёт только расписание
доставки: `BOOKING` и `GAME` уходят с `Urgency: high` и TTL 3600 секунд, остальные — с обычным
приоритетом и TTL контура. Service worker показывает уведомление с иконкой приложения, монохромным
badge, вибрацией и `renotify`, но баннер, его приоритет, звук и режим «Не беспокоить» определяет
операционная система пользователя, а не PadlHub.

Иконка и badge — брендовые артефакты, а не placeholder: `phub-notification-icon-{192,512}.png`
содержат белый lock-up логотипа PadlHub на фирменном градиенте (полный логотип, потому что часть
платформ обрезает аватар по кругу), `phub-notification-badge-72.png` — монохромный силуэт ракетки на
прозрачном фоне, так как Android тонирует badge в системный цвет. Та же команда
`npm run notifications:icons:generate` собирает и артворк устанавливаемого веб-приложения:
`phub-app-icon-{192,512}.png` для манифеста и `apple-touch-icon.png` 180×180 без прозрачности
(iOS накладывает его на фон экрана «Домой»). Тест `apps/web/src/notification-sw-assets.test.ts`
пинует размеры, градиент, белую заливку lock-up, монохромность badge, содержимое манифеста и
непрозрачность touch-иконки.

Операционная статистика доставки живёт только в наших таблицах: у Web Push нет ни дашборда
провайдера, ни отчёта о доставке, кроме факта приёма пуш-сервисом. `GET
/admin/api/v1/{tenantKey}/notifications/delivery-stats?days=N` (1–90) отдаёт по окну: счётчики по
каналам (в очереди, принято, failed, dead, suppressed, ещё в работе, медиана времени до приёма), топ
кодов ошибок, последние кампании с их счётчиками и исходами доставок, и текущее состояние endpoint'ов
(`ACTIVE`/`INVALID`/`REVOKED`/`SUSPENDED_POLICY`). Разрез по платформам берётся из самих endpoint'ов:
адрес расшифровывается в процессе API и сразу сводится к сервису (`CHROME` для FCM, `SAFARI` для
Apple, `OTHER` для явно разрешённого контура), поэтому хеш-адрес и полный URL по-прежнему не покидают
сервер. Тот же разрез отдаётся в списке подписчиков (`platforms`) вместе с `lastConfirmedAt` — по ним
и видно, что устройство давно не подтверждалось и потому не является живым получателем. Показы и открытия приходят от клиента: service worker после успешного `showNotification`
отправляет квитанцию `DISPLAYED`, а по клику — `OPENED`. Сессия ему недоступна (он часто работает при
закрытой странице), поэтому в payload каждого пуша воркер кладёт **подписанный токен квитанции** для
конкретной доставки: ключ выводится доменным разделением из keyring'а endpoint'ов, который есть и у
API, и у воркера, так что новых секретов и ключей конфигурации не появляется. Токен авторизует ровно
одно действие — записать квитанцию для своей доставки; чужой тенант, истёкший срок, подделанная подпись
и мусор дают один и тот же отказ `NOTIFICATION_RECEIPT_INVALID`, а повтор идемпотентен по паре
(доставка, тип). Если токен не помещается в бюджет payload вместе с deep-link'ом, побеждает само
уведомление: для этой доставки воронка просто теряется. Обе метрики — **нижняя оценка**: iOS может не
запустить воркер в фоне, а «показано» не равно «прочитано».

Web Push на iOS и iPadOS имеет собственные ограничения, и они не выводятся из поддержки браузера:
iOS доставляет push **только** веб-приложению, добавленному на экран «Домой» (iOS 16.4+), поэтому
приложение объявляет `manifest.webmanifest` с `display: standalone`, а клиент в обычной вкладке
Safari возвращает состояние `needs_install` и показывает шаги «Поделиться → На экран „Домой“»
вместо кнопки подписки: запрос разрешения в такой вкладке всё равно не сработал бы, а решение
пользователя одноразовое. Safari подписывается через `https://web.push.apple.com`, и этот origin
должен быть в `WEB_PUSH_ALLOWED_ENDPOINT_ORIGINS` контура — иначе подписка отвергается как
`WEB_PUSH_SUBSCRIPTION_INVALID` (та же проверка повторяется воркером перед отправкой). Иконку и
badge iOS игнорирует: в баннере видно только иконку установленного приложения, поэтому бюджет
заголовка для iOS (до 30 символов) и есть то, что реально влияет на вид уведомления. Service worker
показывает баннер через общий помощник с минимальным fallback: если движок отвергнет неподдерживаемый
параметр (например, `vibrate`), уведомление всё равно показывается.

Лимит 300 символов в `notifications.intents` — это лимит in-app inbox, а не баннера. Системный баннер
усекает сама ОС/браузер по ширине экрана: ориентир для оператора — заголовок до 40 символов (до 30,
чтобы уместиться на iOS), текст до 120 символов; in-app показывает полный текст, поэтому более
длинное сообщение не теряется, а только обрезается в баннере. Формат усечения (одна строка,
многоточие, несколько строк на Android) выбирает платформа, и PadlHub на него не влияет.
