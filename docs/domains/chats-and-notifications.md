...
Статус: целевая архитектура, expand-only фундамент и первый feature-gated in-app vertical slice.
Остальные публичные операции остаются закрытыми, пока не реализованы авторизация, идемпотентность,
аудит и обработчики соответствующего вертикального среза.
...
/notifications/read-cursor` и типизированный SDK. Web Push/APNs/FCM пока представлены общим delivery
port и раздельными выключенными tenant gates; provider adapters и endpoint API относятся к
следующему этапу.
...
- `POST|DELETE /{tenantKey}/notification-endpoints` для текущей Web/iOS/Android установки
...
5. **In-app notifications:** templates, rules, intents, preferences и inbox. Первый пользовательский
   срез реализован и закрыт tenant gate; UI и ЦУП-управление templates/rules остаются следующими
   задачами.
6. **Web/iOS/Android push:** encrypted endpoints, VAPID/APNs/FCM adapters, token rotation, quiet
   hours и честные delivery/display/open receipts.
