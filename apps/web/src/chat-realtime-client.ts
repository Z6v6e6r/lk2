import type { RealtimeTicket } from '@phub/api-sdk';

export type ChatRealtimeState = 'connecting' | 'connected' | 'fallback';
...
export function connectChatRealtime(options: {
  ...
  readonly getTicket: () => Promise<RealtimeTicket>;
  readonly getAfterSequence: () => number;
  readonly onRecoveryRequired: (afterSequence: number) => void;
  readonly onStateChange?: (state: ChatRealtimeState) => void;
