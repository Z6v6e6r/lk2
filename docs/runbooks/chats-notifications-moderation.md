...
1. Применить миграции `0093`, `0094` и `0095`. `0093` добавляет проверки как `not valid`, а `0095`
   их валидирует: расширение и валидация разделены, чтобы медленный скан существующей таблицы не
   откатил expand-only файл под `statement_timeout`. Убедиться, что все три применились и повторный
   прогон migrator ничего не меняет.
...
Поведение при сбоях сканирования и удаления:

- сбой `getExact`, ClamAV или объекта переводит ассет не в `REJECTED`, а в повтор с максимальной
  задержкой (`phub.worker.messaging.media.scan_failed` считает попытки сверх бюджета
  `CHAT_MEDIA_SCAN_MAX_ATTEMPTS`). `REJECTED` остаётся только для обнаруженной угрозы и
  недопустимого содержимого, поэтому отправитель не получает ложное «файл не прошёл проверку
  безопасности» из-за недоступности сканера;
- скан, который не завершился за 24 часа, освобождается через expiry: ассет становится `EXPIRED`,
  объекты ставятся в GC, а отправитель видит истёкшую загрузку и может повторить её;
- ассет становится `PURGED` только когда в `messaging.media_gc_jobs` не осталось ни одного задания
  по нему, включая dead-letter. Dead-letter означает, что байты всё ещё в бакете: строка остаётся
  `EXPIRED`, её видно по `phub.worker.messaging.media.gc_dead`, и она требует ручного разбора
...
остаётся в очереди ЦУП и может быть решена повторно (`HIDE`→`DISMISS` оставляет сообщение скрытым
на кейсе `DISMISSED`; `DISMISS`→`RESTORE` даёт открытый кейс вне очереди). До исправления
permissions `chat.moderation.read/decide` на контуре не выдаются, а жалобы пользователей остаются
durable в `moderation.reports` и разбираются SQL-запросом.
