...
Статус: целевая архитектура, expand-only фундамент, feature-gated direct chat M1 + realtime M2,
GAME contextual chat M3.1, in-app, Web Push/VAPID и ручная отправка из ЦУП. Остальные публичные
операции остаются закрытыми, пока не реализованы авторизация, идемпотентность, аудит и обработчики
соответствующего вертикального среза.
...
`/chats`. M2 добавляет 30-секундные одноразовые tickets, membership-проверку при подписке,
identifier-only fanout из RabbitMQ и reconnect/gap recovery через HTTP `afterSequence`. Все четыре
...
M3.1 реализует worker-owned проектор `GAME`: доменное событие содержит только PadlHub UUID,
...
Авторизованные game cards получают nullable ссылку на чат, список диалогов поддерживает
дискриминированные `DIRECT | GAME`, а bounded backfill по умолчанию работает в dry-run.
...
Это целевая карта. В M1/M2 реализованы первые четыре операции, read cursor и realtime для
`DIRECT`; M3.1 использует те же history/send/read/realtime операции для `GAME`, сохраняя отдельный
gate создания direct-диалога. Остальные routes остаются закрытыми до своего вертикального среза.
...
3. **Realtime:** single-use tickets, direct membership subscriptions, identifier-only fanout и
   sequence-gap recovery реализованы за отдельным tenant gate; HTTP остаётся fallback.
