...
`CHAT_PARTICIPANT_CHAT_ACCESS_REQUIRED` instead of receiving a thread they could never open. While
`BETA_FULL_CLIENT_ACCESS_ENABLED=true` every client token issuance converges the stored profile with the
client catalog, so a player who has logged in or refreshed their session on the current image already
holds the grant; a session captured from an earlier image still needs the backfill. Prove the grant
before spending the approved mutation window.
