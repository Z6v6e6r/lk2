...
export const homeRecommendationPromotionSchema = z
  .object({
    id: uuid,
    title: z.string().min(1).max(120),
    route: promotionHref,
    ...
    footerText: z.string().min(1).max(100).optional(),
  })
  .strict();
...
export type HomeRecommendationPromotionDeck = z.infer<typeof homeRecommendationPromotionDeckSchema>;
...
function normalizePromotionSlots(value: unknown): unknown {
  if (isRecord(value) && 'hero' in value && 'standard' in value) {
    return {
      ...value,
      recommendationStrip: value.recommendationStrip ?? { repeatEveryCards: 4, items: [] },
      recommendationCard: value.recommendationCard ?? { repeatEveryCards: 6, items: [] },
    };
  }
