...
import {
  GAME_NOTIFICATION_DEFINITIONS,
  GAME_NOTIFICATION_LOCALE,
  GAME_NOTIFICATION_REQUEST_HASH,
  GAME_NOTIFICATION_RULE_ACTIVE,
  GAME_NOTIFICATION_RULE_CHANNEL_OVERRIDE,
  GAME_NOTIFICATION_RULE_KEY_SUFFIX,
  GAME_NOTIFICATION_RULESET_VERSION,
  GAME_NOTIFICATION_TEMPLATE_ACTIVE,
  GAME_NOTIFICATION_TEMPLATE_CATEGORY,
  GAME_NOTIFICATION_TEMPLATE_CHANNELS,
  GAME_NOTIFICATION_TEMPLATE_DEEP_LINK,
  GAME_NOTIFICATION_TEMPLATE_VERSION,
} from '@phub/notifications';
import type { PoolClient, QueryResultRow } from 'pg';

const CONFIRMATION_TOKEN = 'APPLY_GAME_NOTIFICATION_RULESET';
...
function templateMatches(
  row: TemplateRow,
  definition: (typeof GAME_NOTIFICATION_DEFINITIONS)[number],
): boolean {
  return (
    row.category === GAME_NOTIFICATION_TEMPLATE_CATEGORY &&
    JSON.stringify(row.channels) === JSON.stringify(GAME_NOTIFICATION_TEMPLATE_CHANNELS) &&
    row.title_template === definition.title &&
    row.body_template === definition.body &&
    row.deep_link_template === GAME_NOTIFICATION_TEMPLATE_DEEP_LINK
  );
}
...
try {
  ...
  if (current.previous && current.previous.request_hash !== GAME_NOTIFICATION_REQUEST_HASH) {
  ...
  const preview = {
    ...
    tenantId,
    actorId,
    rulesetVersion: GAME_NOTIFICATION_RULESET_VERSION,
    locale: GAME_NOTIFICATION_LOCALE,
    idempotencyKey,
    replay: Boolean(current.previous),
    inAppRuntimeEnabled: current.runtime?.in_app_enabled ?? false,
    runtimeChangedByThisCommand: false,
    definitions: GAME_NOTIFICATION_DEFINITIONS.map((definition) => ({
      key: definition.key,
  ...
  if (confirm !== CONFIRMATION_TOKEN) {
    ...
    const result = await withTenantTransaction(pool, tenantId, async (client) => {
      ...
      await client.query('select pg_advisory_xact_lock(hashtextextended($1, 0))', [
        `notification-ruleset:${tenantId}:${GAME_NOTIFICATION_RULESET_VERSION}`,
      ]);
      await assertNotificationAdminAccess(client, tenantId, actorId);
      ...
      if (previous) {
        if (previous.request_hash !== GAME_NOTIFICATION_REQUEST_HASH) {
          throw new Error('IDEMPOTENCY_KEY_REUSED');
        }
      ...
      for (const definition of GAME_NOTIFICATION_DEFINITIONS) {
        await client.query(
        ...
        const template = await queryOne<TemplateRow>(
          ...
          `select id, category, channels, title_template, body_template, deep_link_template
            ...
            for update`,
          [tenantId, definition.key, GAME_NOTIFICATION_TEMPLATE_VERSION, GAME_NOTIFICATION_LOCALE],
        );
        if (!template) throw new Error('GAME_NOTIFICATION_TEMPLATE_WRITE_LOST');
        if (!templateMatches(template, definition)) {
          throw new Error(`GAME_NOTIFICATION_TEMPLATE_VERSION_CONFLICT:${definition.key}`);
        }
        await client.query(
          ...
          [tenantId, definition.key, GAME_NOTIFICATION_LOCALE, template.id],
        );
        await client.query(
          `update notifications.templates set active = $3
            where tenant_id = $1 and id = $2`,
          [tenantId, template.id, GAME_NOTIFICATION_TEMPLATE_ACTIVE],
        );
        ...
        const rule = await queryOne<IdRow>(
          ...
          [
            tenantId,
            `${definition.key}.${GAME_NOTIFICATION_RULE_KEY_SUFFIX}`,
        ...
        if (!rule) throw new Error('GAME_NOTIFICATION_RULE_WRITE_LOST');
        ruleIds.push(rule.id);
      }

      const appliedResult = {
        rulesetVersion: GAME_NOTIFICATION_RULESET_VERSION,
        templateVersion: GAME_NOTIFICATION_TEMPLATE_VERSION,
        locale: GAME_NOTIFICATION_LOCALE,
        templateIds,
        ruleIds,
      ...
      await client.query(
        `insert into audit.audit_log (
         ...
         ) values ($1, $2, 'GAME_NOTIFICATION_RULESET_PROVISIONED', 'TENANT', $1,
                   'SUCCESS', $3, $4::jsonb)`,
        [
          tenantId,
          actorId,
          `game-notification-ruleset:${idempotencyKey}`,
          JSON.stringify(appliedResult),
        ],
      );
