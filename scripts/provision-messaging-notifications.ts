/**
 * Provision the direct-chat notification ruleset for one tenant: one template and one active rule
 * per messaging source event. Messaging events stay identifier-only, so the rule resolves its
 * recipients from `recipientUserIds` and the rendered body never quotes message content.
 *
 * Dry-run is the default. Applying requires `--confirm=APPLY_MESSAGING_NOTIFICATION_RULESET`, an
...
import {
  ...
  MESSAGING_NOTIFICATION_RULE_ACTIVE,
...
const CONFIRMATION_TOKEN = 'APPLY_MESSAGING_NOTIFICATION_RULESET';
...
try {
  ...
  if (current.previous && current.previous.request_hash !== MESSAGING_NOTIFICATION_REQUEST_HASH) {
  ...
  if (confirm !== CONFIRMATION_TOKEN) {
    ...
    const result = await withTenantTransaction(pool, tenantId, async (client) => {
      ...
      await client.query('select pg_advisory_xact_lock(hashtextextended($1, 0))', [
        `notification-ruleset:${tenantId}:${MESSAGING_NOTIFICATION_RULESET_VERSION}`,
      ]);
      await assertNotificationAdminAccess(client, tenantId, actorId);
      ...
      if (previous) {
        if (previous.request_hash !== MESSAGING_NOTIFICATION_REQUEST_HASH) {
          throw new Error('IDEMPOTENCY_KEY_REUSED');
        }
      ...
      for (const definition of MESSAGING_NOTIFICATION_DEFINITIONS) {
        ...
        if (!template) throw new Error('MESSAGING_NOTIFICATION_TEMPLATE_WRITE_LOST');
        if (!templateMatches(template, definition)) {
          throw new Error(`MESSAGING_NOTIFICATION_TEMPLATE_VERSION_CONFLICT:${definition.key}`);
        }
        await client.query(
          ...
          [tenantId, definition.key, MESSAGING_NOTIFICATION_LOCALE, template.id],
        );
        await client.query(
          `update notifications.templates set active = $3
            where tenant_id = $1 and id = $2`,
          [tenantId, template.id, MESSAGING_NOTIFICATION_TEMPLATE_ACTIVE],
        );
        ...
        if (!rule) throw new Error('MESSAGING_NOTIFICATION_RULE_WRITE_LOST');
      ...
      await client.query(
        `insert into audit.audit_log (
         ...
         ) values ($1, $2, 'MESSAGING_NOTIFICATION_RULESET_PROVISIONED', 'TENANT', $1,
                   'SUCCESS', $3, $4::jsonb)`,
        [
          tenantId,
          actorId,
          `messaging-notification-ruleset:${idempotencyKey}`,
          JSON.stringify(appliedResult),
        ],
      );
