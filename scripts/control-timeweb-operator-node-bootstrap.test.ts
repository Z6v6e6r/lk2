...
describe('Timeweb operator Node bootstrap controller', () => {
  ...
  it('accepts an owned system launcher symlink and rejects unsafe symlink states', () => {
    const program = [
      ...
      'module = importlib.util.module_from_spec(spec)',
      'spec.loader.exec_module(module)',
      'def inode(file_type, mode, uid=0):',
      '    return SimpleNamespace(st_mode=file_type | mode, st_uid=uid)',
      'class Resolved:',
      '    def __init__(self, target, stat_error=False, identity="/usr/bin/python3.14"):',
      '        self.target = target',
      ...
      '        return self.target',
      'class LauncherPath:',
      '    def __init__(self, link, target=None, resolution_error=False, target_error=False, resolved_identity="/usr/bin/python3.14"):',
      '        self.link = link',
      '        self.target = target',
      '        self.resolution_error = resolution_error',
      '        self.target_error = target_error',
      '        self.resolved_identity = resolved_identity',
      '    def lstat(self):',
      '        return self.link',
      '    def resolve(self, strict=False):',
      '        assert strict is True',
      '        if self.resolution_error:',
      '            raise OSError("dangling launcher")',
      '        return Resolved(self.target, self.target_error, self.resolved_identity)',
      'def assert_stop(path):',
      '    try:',
      '        module.require_secure_launcher(path)',
      '    except module.Stop as error:',
      '        assert str(error) == "launcher_security"',
      '    else:',
      '        raise AssertionError("unsafe launcher accepted")',
      'system_symlink = inode(stat.S_IFLNK, 0o777)',
      'safe_target = inode(stat.S_IFREG, 0o755)',
      'module.require_secure_launcher(LauncherPath(system_symlink, safe_target))',
      'unexpected_safe_target = inode(stat.S_IFREG, 0o755)',
      'unexpected_resolution = module.require_secure_launcher(LauncherPath(system_symlink, unexpected_safe_target, resolved_identity="/opt/unexpected-but-secure/python3"))',
      'assert unexpected_resolution.target is unexpected_safe_target',
      'assert unexpected_resolution.identity == "/opt/unexpected-but-secure/python3"',
      'module.require_secure_launcher(LauncherPath(inode(stat.S_IFREG, 0o755), safe_target))',
      'assert_stop(LauncherPath(inode(stat.S_IFLNK, 0o777, uid=1000), safe_target))',
      'assert_stop(LauncherPath(system_symlink, resolution_error=True))',
      'assert_stop(LauncherPath(system_symlink, safe_target, target_error=True))',
      'assert_stop(LauncherPath(system_symlink, inode(stat.S_IFREG, 0o755, uid=1000)))',
      'assert_stop(LauncherPath(system_symlink, inode(stat.S_IFREG, 0o775)))',
      'assert_stop(LauncherPath(system_symlink, inode(stat.S_IFREG, 0o757)))',
      'assert_stop(LauncherPath(system_symlink, inode(stat.S_IFDIR, 0o755)))',
      'assert_stop(LauncherPath(inode(stat.S_IFREG, 0o775), safe_target))',
      'assert_stop(LauncherPath(inode(stat.S_IFDIR, 0o755), safe_target))',
      'class ContractPath:',
      '    def __init__(self, value):',
      '        self.value = value',
      '    def __eq__(self, other):',
      '        return isinstance(other, ContractPath) and self.value == other.value',
      '    def lstat(self):',
      '        return system_symlink',
      '    def resolve(self, strict=False):',
      '        assert strict is True',
      '        return Resolved(safe_target)',
      'module.Path = ContractPath',
      'module.sys.executable = "/usr/bin/python3"',
      'contract = {"launcher": {"path": "/usr/bin/python3", "major": 3, "packageOwner": "python3-minimal"}, "apt": {"dpkgQueryBinary": "/usr/bin/dpkg-query"}}',
      'calls = []',
      'module.run = lambda args: calls.append(args) or "python3-minimal: /usr/bin/python3"',
      'module.validate_launcher(contract)',
      'assert calls == [["/usr/bin/dpkg-query", "-S", "/usr/bin/python3"]]',
      'module.run = lambda _args: "unexpected-owner: /usr/bin/python3"',
      'try:',
      '    module.validate_launcher(contract)',
      'except module.Stop as error:',
      '    assert str(error) == "launcher_package_owner"',
      'else:',
      '    raise AssertionError("unexpected launcher package owner accepted")',
    ].join('\n');
    const result = spawnSync('python3', ['-I', '-S', '-B', '-c', program, resolve(controller)], {
      encoding: 'utf8',
      env: { PATH: process.env.PATH ?? '/usr/bin:/bin' },
    });
