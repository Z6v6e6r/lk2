...
describe('Timeweb operator Node bootstrap controller', () => {
  ...
  it('accepts an owned system launcher symlink and rejects unsafe symlink states', () => {
    const program = [
      ...
      'class Resolved:',
      '    def __init__(self, target, stat_error=False):',
      '        self.target = target',
      ...
      '        return self.target',
      'class LauncherPath:',
      '    def __init__(self, link, target=None, resolution_error=False, target_error=False):',
      '        self.link = link',
      '        self.target = target',
      ...
      '        return Resolved(self.target, self.target_error)',
