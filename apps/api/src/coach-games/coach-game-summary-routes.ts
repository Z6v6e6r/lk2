import type { PublicCoachGameSummary } from '@phub/viva-adapter';
...
export function registerCoachGameSummaryRoutes(
  app: FastifyInstance,
  options: {
    readonly source?: CoachGameSummarySource;
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      if (
        ...
        return sendApiError(
          ...
          400,
          'INVALID_REQUEST',
          'Некорректные фильтры поиска игр с тренером.',
        );
      }
      if (!options.source) {
        return sendApiError(
          ...
          reply,
          503,
          'COACH_GAME_DISCOVERY_UNAVAILABLE',
          'Игры с тренером временно недоступны.',
        );
      }
      const source = options.source;
      try {
        const pages: PublicCoachGameSummary[][] = [];
        ...
        const items = pages
          ...
          .map((item) => {
            ...
            return {
              ...
              trainer: {
                ...item.trainer,
                avatarUrl: `/public/api/v1/${encodeURIComponent(tenantKey)}/coach-games/${encodeURIComponent(item.id)}/trainer-avatar`,
              },
            };
          });
