...
2. `POST /admin/api/v1/{tenantKey}/notifications/recipients/resolve` нормализует до 100 телефонов,
   ...
   приоритет: сначала авторитетная provider-привязка
   `integration.external_entity_map` (`VIVA`/`legacy_viewer_phone`, уникальна внутри tenant и
   fail-closed на конфликтах), и только для телефонов, которых провайдер не сообщил, — колонка
   `profile.user_summaries.phone_e164`, которая означает подтверждённый вход по телефону и может
   остаться от legacy-импорта на другом аккаунте. Телефон, привязанный провайдером к неактивному
   пользователю, не откатывается на устаревшую строку, а два пользователя с одним телефоном дают
   unresolved, а не произвольный выбор.
