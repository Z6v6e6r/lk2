...
прозрачном фоне, так как Android тонирует badge в системный цвет. Оба файла воспроизводимы из
`apps/web/src/assets/padlhub-logo.svg` командой
`npm run notifications:icons:generate`; тест `apps/web/src/notification-sw-assets.test.ts`
пинует размеры, градиент, белую заливку lock-up и монохромность badge.
